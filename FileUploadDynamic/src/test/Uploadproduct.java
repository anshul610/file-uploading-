		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	      ...
	      while ( i.hasNext())
	      {
	         ...
	         {// Get the uploaded file parameters
	           ...
	           if(fieldName.equals("category")) {category =fi.getString();System.out.println(category);}
	           if(fieldName.equals("pname")) { productname=fi.getString(); System.out.println(productname); }
	           if(fieldName.equals("price")){  productprice=fi.getString();System.out.println(productprice); }
	           if(fieldName.equals("desc"))  { productdesc=fi.getString(); System.out.println(productprice); }
	         } else
	          {     String fieldName = fi.getFieldName();
	      ...
	      try {
	    	  ...
	    	  PreparedStatement ps=con.prepareStatement("insert into product(category,pname,price,description,image) value(?,?,?,?,?)");//placeholder
	    	   ps.setString(1, category); ps.setString(2,productname); ps.setString(3,productprice); ps.setString(4,productdesc);
	    	    ps.setString(5,filename);
	    	    int y=0;y=ps.executeUpdate();
	    	    if(y!=0)out.println("uploaded successfully...");
	            con.close();
